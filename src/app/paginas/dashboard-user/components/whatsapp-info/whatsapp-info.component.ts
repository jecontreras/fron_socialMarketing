...
export class WhatsappInfoComponent implements OnInit {
  ...
  Header:any = [ 'Acciones','Titulo','Numero','Estado', 'Creado' ];
  $:any;
  public datoBusqueda = '';
  ...
  async delete(obj:any){
    ...
    return new Promise(resolve=>{
      this._whatsappInfo.editar(data).subscribe((res:any)=>{
  ...
  editar(obj:any){
    this.Router.navigate(['/dashboard/whatsappInfoform', obj.id]);
  }
  ...
  onScroll(){
   ...
   cargarTodos() {
     ...
     this._whatsappInfo.get(this.query)
     .subscribe(
       (response: any) => {
